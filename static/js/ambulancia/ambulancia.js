...
function obtenerValoresAmbulancia() {
  ...
  const input3 = document.getElementById("placaTxt").value;
  const input4 = document.getElementById("anioTxt").value;
  ...
  if (input1.trim() === "" || input3.trim() === "") {
    Swal.showValidationMessage(
      `Request failed: Por favor ingresa un Nombre de Ambulancia y una placa.`
    );
  } else if (!numericRegex.test(input4) || !numericRegex.test(input5)) {
    Swal.showValidationMessage(
      `Request failed: Por favor ingresa un valor numérico válido para el año y la capacidad.`
    );
  } else {
...
function llenarTablaAmbulancia(datos) {
  ...
  datos.forEach((ambulancia) => {
    ...
    td2.textContent = ambulancia.placa;
    td3.textContent = ambulancia.modelo;
    td4.textContent = ambulancia.marca;
    td5.textContent = ambulancia.anio;
    ...
    const btnViewer = crearBotonAmbulanciaView(() =>
      ViewAmbulancia(ambulancia.id)
    );

    const contenedorBotones = document.createElement("div");
    contenedorBotones.classList.add("d-flex", "justify-content-end", "gap-2");
    contenedorBotones.appendChild(btnViewer);
...
function crearBotonAmbulanciaView(onClick) {
  const boton = document.createElement("button");
  boton.textContent = "";
...
function ViewAmbulancia(id) {
  Swal.fire("Esta Seguro?", `Eliminar Ambulancia de ${id}!`, "warning");
}
...
async function update_Ambulancia(id_ambulancia) {
  ...
  const { value: formValues } = await Swal.fire({
    ...
    html: `<div class="form-floating">
            ...
            <div class="form-floating">
              <input id="placaTxt" type="text" value="${ambulancia.placa}" class="form-control">
              <label for="placaTxt" class="form-label">Placa:</label>
            </div>
          <div class="row">
            <div class="form-floating col">
              <input id="anioTxt" type="number" value="${ambulancia.anio}" class="form-control">
              <label for="anioTxt" class="form-label">Año:</label>
            </div>
            <div class="form-floating col">
