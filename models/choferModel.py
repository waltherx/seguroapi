...
class ChoferModel:
    ...
    @classmethod
    def get_chofers(self, ci):
        try:
            sQuery = f"SELECT idch, licencia, categoria, estado, id_ambulancia FROM public.chofer, public.ambulancia  where chofer.id_ambulancia = ambulancia.idam  and chofer.id_ambulancia =  {ci};"
            connection = get_connection()
            chofers = []
            with connection.cursor() as cursor:
